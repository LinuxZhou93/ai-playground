...
4. 篇幅饱满：为了让报告显得详尽和充实，防止页面有太多空白，你的综合评价字数必须控制在 500-600 字之间，务必加入丰富具体的机械原理（如传动咬合、连杆受力、多级变速）和算法执行（如分支监听、物理传感器参数反馈、闭环调优）等实战细节描述。

输出格式要求必须返回纯合法的 JSON 字符串（不要附带markdown代码块标识），包含以下字段：
{
  "ai_overall_report": "一段代表学术委员会与主审导师团队针对该学员八个维度以及本学期在简单机械结构搭建、流程图/图形化编程逻辑、电子元器件交互等表现的详细综合定性评估（500-600字），要显得极其专业、充满行业洞见、且对家长有较高的温度感知，结合实验室具体的物理和算法课程元素。字数必须足够饱满，防止页面空白。",
  "ai_recommendations": [
