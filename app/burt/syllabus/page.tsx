...
const firstSixWeeks = [
  ...
  },
  {
    week: 'Week 2',
    title: 'AI 项目原型',
    desc: '继续完善 SAT 学习网页，理解前后端基础概念，体验 Computer Use、Chrome 浏览器控制和定时任务。',
    icon: BrainCircuit,
  },
  {
    week: 'Week 3',
      ...
      <nav className="fixed top-0 w-full backdrop-blur-xl bg-neutral-950/80 border-b border-neutral-800 z-50">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center gap-4">
          <Link
            prefetch={false}
            href="/burt"
            className="p-2 -ml-2 rounded-lg hover:bg-neutral-800 transition-colors text-neutral-400 hover:text-white flex items-center justify-center"
          >
      ...
      <main className="pt-32 px-6 lg:px-8 max-w-6xl mx-auto">
        <motion.div initial="hidden" animate="visible" variants={fadeIn} className="mb-16">
          ...
          <p className="text-neutral-400 text-lg max-w-3xl leading-relaxed">
            本页用于说明第一阶段课程如何推进：先完成 AI 工具链和 VibeCoding 的高频实操，再进入
            ROS2、仿真、机械臂与具身智能任务，最后通过线上辅导把代码、实验和文档持续沉淀。
          </p>
        </motion.div>
        ...
        >
          {hours.map((item) => (
            <motion.div
              key={item.label}
              variants={fadeIn}
              className="rounded-3xl bg-neutral-900 border border-neutral-800 p-6"
            >
        ...
        >
          <motion.div variants={fadeIn} className="mb-10">
            ...
            <p className="text-neutral-400 max-w-3xl leading-relaxed">
              模块不是彼此割裂的章节，而是会随着项目推进交叉出现：AI
              工具链负责提高实现效率，机器人系统负责把算法落到物理任务，文档表达负责把过程保存下来。
            </p>
          </motion.div>
                ...
                >
                  ...
                  <div className="grid sm:grid-cols-2 gap-2 mb-5">
                    {module.points.map((point) => (
                      <div
                        key={point}
                        className="flex items-start gap-2 rounded-xl bg-neutral-950 border border-neutral-800 px-3 py-2"
                      >
              ...
              {firstSixWeeks.map((week) => {
                ...
                return (
                  <motion.div
                    key={week.week}
                    variants={fadeIn}
                    className="grid lg:grid-cols-[80px_1fr] gap-5 items-start"
                  >
        ...
        <motion.section
          ...
          viewport={{ once: true }}
          variants={fadeIn}
          className="rounded-[2rem] bg-neutral-900 border border-neutral-800 p-8 lg:p-10"
        >
          <div className="flex flex-col lg:flex-row gap-8 lg:items-center justify-between">
            <div>
              ...
              <p className="text-neutral-400 leading-relaxed max-w-2xl">
                如果对课时结构、模块顺序和阶段产出方向没有问题，就可以按 6 月 18
                日开课节奏启动首批工具、账号、代码仓库和软硬件材料准备。
              </p>
            </div>
            <Link
              prefetch={false}
