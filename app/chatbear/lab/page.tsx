      ...
      <main className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        ...
        <aside className="w-full lg:w-72 bg-white border-r border-gray-100 flex flex-col shrink-0">
          <div className="p-6 border-b border-gray-100">
            <h2 className="text-sm font-black text-gray-900 uppercase tracking-widest flex items-center gap-2">
              <Code2 size={16} className="text-blue-500" /> 项目文件
            </h2>
          </div>
          <div className="flex-1 overflow-y-auto p-4">
            <div className="space-y-1">
              {['main.py', 'robot_config.json', 'perception.py'].map((file, i) => (
                <div key={i} className={`px-4 py-2.5 rounded-xl text-sm cursor-pointer flex items-center gap-3 ${i === 0 ? 'bg-blue-50 text-blue-700 font-bold' : 'text-gray-500 hover:bg-gray-50'}`}>
          ...
          <div className="p-6 border-t border-gray-100">
             <div className="bg-yellow-50 p-4 rounded-2xl border border-yellow-100 flex gap-3">
                ...
                <p className="text-[10px] text-yellow-800 leading-tight">
                  ...
                  你可以修改左侧的代码，然后点击右上角的“运行仿真”查看结果哦！
                </p>
             </div>
          </div>
        </aside>
        ...
        <div className="flex-1 flex flex-col min-w-0">
              ...
              <div className="flex items-center gap-2 text-xs font-medium text-gray-500">
                {isPyodideLoading ? (
                  ...
                  <>
                    ...
                    <span className="text-green-600">Python 环境就绪</span>
                  </>
                )}
              </div>
            </div>
            <div className="flex items-center gap-3">
              <button className="p-2 text-gray-400 hover:text-black transition-colors"><Save size={20} /></button>
              ...
              <button 
                ...
                className="px-6 py-2 bg-blue-600 text-white rounded-xl font-bold text-sm flex items-center gap-2 hover:bg-blue-700 disabled:bg-gray-400 transition-all shadow-lg shadow-blue-500/20 active:scale-95"
              >
          ...
          <div className="flex-1 flex flex-col lg:flex-row min-h-0">
            {/* Editor Area */}
            <div className="flex-1 bg-[#1e1e1e] relative pt-4">
               <Editor
                 height="100%"
            ...
            <div className="w-full lg:w-[45%] bg-black relative border-l border-white/5">
              <div className="absolute inset-0 flex items-center justify-center">
                 <div className="text-center">
                    <div className="relative inline-block mb-6">
                       <div className="absolute -inset-8 bg-blue-500/20 rounded-full blur-3xl animate-pulse"></div>
                       <Box size={80} className="text-white/20" />
                    </div>
                    <p className="text-white/40 text-sm font-medium tracking-widest uppercase">3D 仿真视窗开发中</p>
                    <p className="text-white/20 text-xs mt-2">控制台输出已可用</p>
                 </div>
              </div>
              
              {/* Overlay HUD */}
              <div className="absolute top-6 left-6 flex flex-col gap-2">
                 <div className="px-3 py-1 bg-black/40 backdrop-blur-md border border-white/10 rounded-full text-[10px] text-white/80 font-bold flex items-center gap-2">
                    <Layers size={12} /> 物体检测: {isRunning ? <span className="text-green-400">进行中</span> : '开启'}
                 </div>
              </div>
            </div>
          </div>
          ...
          {/* Console Area */}
          <div className="h-48 bg-white border-t border-gray-100 flex flex-col">
             ...
             <div className="flex-1 overflow-y-auto p-4 font-mono text-[12px] text-gray-700 bg-[#f8f9fa]">
                {logs.map((log, i) => (
