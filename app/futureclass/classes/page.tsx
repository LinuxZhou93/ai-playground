...
import { getClassesWithStats, getCourses, addClass } from "../actions";
...
export default function ClassesPage() {
  ...
  const fetchData = () => {
    setLoading(true);
    Promise.all([getClassesWithStats(), getCourses()]).then(([classesData, coursesData]) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="flex gap-2 text-sm">
            <div className="px-3 py-1.5 rounded-lg bg-blue-500/10 text-blue-600 font-semibold flex items-center gap-1.5">
              <BarChart3 className="h-3.5 w-3.5" /> {classes.length} 个班级
            </div>
            <div className="px-3 py-1.5 rounded-lg bg-emerald-500/10 text-emerald-600 font-semibold flex items-center gap-1.5">
              <Users className="h-3.5 w-3.5" /> {totalStudents} 名学员
            </div>
            <div className="px-3 py-1.5 rounded-lg bg-amber-500/10 text-amber-600 font-semibold">
              平均满班率 {avgFillRate}%
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 relative min-h-[300px]">
        {loading ? (
          ...
          <Card key={cls.id} className="hover:shadow-md transition-all shadow-sm border-none overflow-hidden group">
              ...
              <div className="grid grid-cols-2 gap-6 mt-5">
                ...
                <div className="space-y-2">
                  ...
                  <div className="h-2 bg-muted rounded-full overflow-hidden">
                    <div 
                      className={`h-full rounded-full transition-all duration-500 ${getProgressColor(cls.fillRate)}`}
                      style={{ width: `${Math.min(100, cls.fillRate)}%` }}
                    />
                  </div>
                </div>
              </div>
                ...
                {cls.studentNames.length === 0 ? (
                  ...
                  <div className="flex flex-wrap gap-2">
                    {cls.studentNames.slice(0, 8).map((name: string, i: number) => (
                      <div key={i} className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-muted/50 text-xs font-medium">
