...
XMP 不是一个孤立的 AI 玩具或备课工具，而是连接教研、课堂、儿童交互、成长证据、家园共育、园所经营、边缘设备与数据治理的幼教 AI 操作系统。

## 用户与价值

| 角色       | 原有断点                       | XMP 的核心价值                                       |
| ---------- | ------------------------------ | ---------------------------------------------------- |
| 园所管理者 | 教学、设备、经营与交付信息分散 | 在一个总控台看见教学质量、运营风险和实施进度         |
| 教研负责人 | 内容生产慢，课程与现场脱节     | 从教研意图生成结构化教案，并保留审核、版本和复用能力 |
| 主班教师   | AI 建议难以进入真实课堂        | 教师掌舵，多端协作，现场建议可接受、忽略或接管       |
...
| ---------------------------------------------------------------------------------- | --------------------------------------------- | -------------- | -------------------------------------------- |
| `app/edu/generator`、`app/api/generate/*`、统一模型与媒体提供器                    | AI 课程工厂：简报、生成、阶段、材料、审核     | 本地交互演示   | 接入课程版本库、生成任务与审核状态           |
| `components/stage.tsx`、课堂生成与语音/视觉链路                                    | 实时课堂：教师台、课堂编排、Copilot、多端状态 | 本地交互演示   | 接入真实课堂会话、设备事件总线与容错策略     |
| 语音、视觉、TTS 与多模型能力                                                       | 奇妙宠：低认知交互、物理静音、教师接管        | 本地交互演示   | 接入边缘运行时、内容安全和设备身份           |
| `erp_growth_archives`、`components/erp/growth-timeline.tsx`、`api/scribe/generate` | 成长智能：证据审核、成长地图、签发报告        | 本地交互演示   | 接入证据对象、教师签名和版本化报告           |
| 家长报告生成与成长档案能力                                                         | 家园共育：周报、家庭任务、反馈回流            | 本地交互演示   | 接入监护关系、消息送达、回执与授权状态       |
| ERP 学员、课程、班级、排课、考勤、库存、财务、员工、线索                           | 园所运营：业务健康、交付里程碑、审批、流水    | 本地交互演示   | 建立 XMP 读写适配层与租户级权限策略          |
...
| Supabase、角色与安全测试底座                                                       | 信任中心：授权、访问、导出、删除、事件与证据  | 本地交互演示   | 完成数据地图、影响评估、审批服务和删除编排器 |
...
4. **信任基础设施**：儿童数据授权、最小访问、到期删除和审计证据内建于产品。
...
| 既有 FutureClass 与 XMP 数据耦合 | 通过适配层复用，不直接污染旧业务         | 定义事件模型、租户边界和迁移方案   |
