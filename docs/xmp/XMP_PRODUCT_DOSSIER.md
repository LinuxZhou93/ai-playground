...
当前本地版本已用统一课堂关联 ID 和追加式本地事件流连接实时课堂、成长审核、家庭发布与设备处置。实时课堂与设备舰队共享一个可持久化状态机：关键心跳丢失会自动暂停并关闭 AI，设备恢复后仍需教师明确恢复，人工接管也必须显式释放。跨页面操作可以在“教学闭环事件链”中复盘；默认关闭的服务端迁移层已经具备追加式事件表、事件级租户 RLS、幂等 API 与离线 Outbox，但尚未在真实园所数据库执行或接入生产身份与设备证书。
