...
def add_styled_title(slide, text, color=MOZI_CYAN):
    ...
    title.text_frame.paragraphs[0].font.size = PptPt(36)
...
def generate_pptx():
    ...
    set_slide_background(slide, MOZI_DARK)
    add_styled_title(slide, "核心使命：培养全栈科技特长生")
    add_color_block(slide, 0.5, 1.2, 0.2, 5, MOZI_CYAN)
    content = slide.placeholders[1]
    content.text = "• 核心基因：自我管理 (Self-Management)\n• 思维底层：第一性原理 (First Principles)\n• 目标群体：具备极客潜质的新初一准学者"
    for p in content.text_frame.paragraphs:
        p.font.color.rgb = MOZI_WHITE
        p.font.size = PptPt(24)

    # Slide 3: Funnel Layout (Flowchart Style)
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    set_slide_background(slide, MOZI_DARK)
    add_styled_title(slide, "活动转化漏斗：从人流到生源")
    ...
    slide = prs.slides.add_slide(prs.slide_layouts[1])
    set_slide_background(slide, MOZI_DARK)
    add_styled_title(slide, "智动前沿：众擎机器人表演")
    content = slide.placeholders[1]
    content.text = "• 表演脚本：动态避障、台阶攀爬、语音应答互动\n• 视觉焦点：双机巡游方阵，制造震撼入场感\n• 传播目标：引发家长朋友圈高频转发，建立品牌势能"
    for p in content.text_frame.paragraphs:
        p.font.color.rgb = MOZI_WHITE
    ...
    slide = prs.slides.add_slide(prs.slide_layouts[1])
    set_slide_background(slide, MOZI_DARK)
    add_styled_title(slide, "原子觉醒：石墨烯电阻云图")
    content = slide.placeholders[1]
    content.text = "• 实验内核：材料科学入门 L01\n• 互动细节：学生亲自动手测绘，理解碳材料导电特性\n• 转化点：测试完毕扫码领取“石墨烯样品礼包”"
    for p in content.text_frame.paragraphs:
        p.font.color.rgb = MOZI_WHITE
    ...
    slide = prs.slides.add_slide(prs.slide_layouts[1])
    set_slide_background(slide, MOZI_DARK)
    add_styled_title(slide, "未来预告：火星/月球基地探索", MOZI_YELLOW)
    content = slide.placeholders[1]
    content.text = "• 课程伏笔：展示地外采矿及生存系统教具\n• 目标导向：吸引对“深空探索”感兴趣的潜在学员\n• 长期价值：建立对暑期及秋季课程的强烈心理预期"
    for p in content.text_frame.paragraphs:
        p.font.color.rgb = MOZI_WHITE
    ...
    slide = prs.slides.add_slide(prs.slide_layouts[1])
    set_slide_background(slide, MOZI_DARK)
    add_styled_title(slide, "权责清单：协同与执行")
    content = slide.placeholders[1]
    content.text = "• 教学部：苏格拉底式提问引导，维护实验专业度\n• 咨询部：精准捕获意向家长，完善学员档案库\n• 工程部：电力、网络及桁架结构的绝对安全保障"
    for p in content.text_frame.paragraphs:
        p.font.color.rgb = MOZI_WHITE
    ...
    slide = prs.slides.add_slide(prs.slide_layouts[1])
    set_slide_background(slide, MOZI_DARK)
    add_styled_title(slide, "风险管控与应急响应")
    content = slide.placeholders[1]
    content.text = "• 硬件故障：备用电池及备用测试仪 100% 就绪\n• 极端客流：设置互动排队线，错峰进行机器人表演\n• 安全红线：机器人与观众保持 1.5 米安全距离"
    for p in content.text_frame.paragraphs:
        p.font.color.rgb = MOZI_WHITE
