...
def run():
    print(f"[{datetime.now()}] 🛸 启动深核超高保真织布机，量产 103~150...")
    
    # 1. Ask Gemini for 48 High-End Disciplines (batch of 48 is ok for JSON list)
    prompt = """请一次性列出 48 个真实的、教育部备案或行业高度发展的前沿交叉学科/新工科专业方向。
要求：
1. 名称必须硬核且真实存在（如：地球信息科学与技术, 辐射防护与核安全, 微电子科学与工程, 系统科学与工程 等）。
2. 每项包含 icon, 简短而极具赛博专业感的 desc。
3. 每项提供3个包含具体前沿技术方向的子主题（subs），含 title 和 desc。
不准重复，严格不准包含科幻虚构要素（如时光机、外星人等）。必须返回纯 JSON 数组，严禁在开头输出 ```json 或任何文字，严格从 [ 开始。"""
    ...
    print(f"[{datetime.now()}] 🎯 全系统注入完毕！103~150 部署上线。")
