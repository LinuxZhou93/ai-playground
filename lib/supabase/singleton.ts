...
import {
  buildSafeSupabaseAnonKey,
  buildSafeSupabaseUrl,
...
export function getSupabase() {
  ...
  if (!isSupabasePublicConfigured) {
    console.warn(
      "[Supabase] Singleton is using the inert local demo endpoint.",
    );
  }
...
// Admin client is intentionally schema-unbound: this legacy repository contains
// several independently migrated schemas that are not represented by one generated
// Database type yet. Runtime payloads remain validated at each protected API edge.
