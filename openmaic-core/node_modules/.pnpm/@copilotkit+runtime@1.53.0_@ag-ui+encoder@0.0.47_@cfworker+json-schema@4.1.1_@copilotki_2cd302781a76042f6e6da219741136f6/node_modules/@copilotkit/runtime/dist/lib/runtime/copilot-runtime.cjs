...
let _copilotkitnext_agent = require("@copilotkitnext/agent");
...
var CopilotRuntime = class {
	constructor(params) {
		...
		const baseRunner = params?.runner ?? new _copilotkitnext_runtime.InMemoryAgentRunner();
	...
	get instance() {
		if (!this._instance) this._instance = new _copilotkitnext_runtime.CopilotRuntime(this.runtimeArgs);
	...
	handleServiceAdapter(serviceAdapter) {
		this.runtimeArgs.agents = Promise.resolve(this.runtimeArgs.agents ?? {}).then(async (agents) => {
			...
			if (isAgentsListEmpty) {
				...
				agentsList.default = new _copilotkitnext_agent.BuiltInAgent({ model });
			}
	...
	createOnAfterRequestHandler(params) {
		return async (hookParams) => {
			...
			if (params?.middleware?.onAfterRequest) params.middleware.onAfterRequest({});
		};
	}
	/**
