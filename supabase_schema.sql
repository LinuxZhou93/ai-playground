...
-- 7. COMMENTS (Neural Nexus)
create table public.comments (
  id bigint generated by default as identity primary key,
  created_at timestamp with time zone default timezone('utc'::text, now()) not null,
  content text not null,
  author_id uuid references public.profiles(id) not null,
  thread_id bigint references public.threads(id) not null,
  parent_id bigint references public.comments(id), -- For nested replies
  upvote_count integer default 0
);

-- 8. LESSON NOTES (User notes per lesson)
...
-- 9. LESSON COMMENTS (Discussion per lesson)
...
create policy "Users can view own notes" on lesson_notes for select using (auth.uid() = user_id);
create policy "Users can insert own notes" on lesson_notes for insert with check (auth.uid() = user_id);
create policy "Users can update own notes" on lesson_notes for update using (auth.uid() = user_id);
...
create policy "Users can create comments" on lesson_comments for insert with check (auth.uid() = user_id);
...
create table public.votes (
  ...
  type text check (type in ('up', 'down')) default 'up',
